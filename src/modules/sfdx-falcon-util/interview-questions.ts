  ...
  return [
    {
      ...
      validate: async userInput => {
        ...
        .catch(readJsonMapError => {
          SfdxFalconDebug.obj(`${dbgNs}provideTm1AnalysisDirectory:readJsonMapError:`, readJsonMapError);
          return;
        ...
        if (typeof tm1AnalysisReport !== 'object' || typeof tm1AnalysisReport.orgInfo !== 'object') {
          ...
          SfdxFalconDebug.obj(`${dbgNs}provideTm1AnalysisDirectory:orgInfo:`, orgInfo);

          // Get the Auth Info for the associated Username.
          const authInfo  = await AuthInfo.create({username: orgInfo.username})
          .catch(authInfoError => {
            SfdxFalconDebug.obj(`${dbgNs}provideTm1AnalysisDirectory:authInfoError:`, authInfoError);
            return;
          }) as AuthInfo;
          ...
          if (typeof authInfo === 'undefined') {
            ...
            SfdxFalconDebug.obj(`${dbgNs}provideTm1AnalysisDirectory:authInfo.AuthFields:`, authInfo.getFields());
...
// ────────────────────────────────────────────────────────────────────────────────────────────────┐
/**
 * @function    provideTm1ExtractionDirectory
 * @param       {string[]}  [promptText]  Optional. Array of strings used as text for each prompt.
 * @returns     {Questions}  An array of Inquirer Question objects.
 ...
 */
// ────────────────────────────────────────────────────────────────────────────────────────────────┘
export function provideTm1ExtractionDirectory(promptText?:string[]):Questions {
  ...
  return [
    {
      ...
      validate: async userInput => {
        ...
        const tm1ExtractionReportFilePath = path.join(userInput, 'tm1-extraction.json');
        const tm1ExtractionReport = await fs.readJsonMap(tm1ExtractionReportFilePath, true)
        .catch(readJsonMapError => {
          SfdxFalconDebug.obj(`${dbgNs}provideTm1ExtractionDirectory:readJsonMapError:`, readJsonMapError);
          return;
        }) as TM1ExtractionReport;

        // Make sure a file was actually found and read into JSON.
        let foundValidFile = true;
        if (typeof tm1ExtractionReport !== 'object' || typeof tm1ExtractionReport.orgInfo !== 'object' || typeof tm1ExtractionReport.actualTm1RecordCounts !== 'object') {
          foundValidFile = false;
        }
        else {
          const orgInfo               = tm1ExtractionReport.orgInfo;
          const actualTm1RecordCounts = tm1ExtractionReport.actualTm1RecordCounts;
          SfdxFalconDebug.obj(`${dbgNs}provideTm1ExtractionDirectory:orgInfo:`, orgInfo);
          SfdxFalconDebug.obj(`${dbgNs}provideTm1ExtractionDirectory:actualTm1RecordCounts:`, actualTm1RecordCounts);

          // Get the Auth Info for the associated Username.
          const authInfo  = await AuthInfo.create({username: orgInfo.username})
          .catch(authInfoError => {
            SfdxFalconDebug.obj(`${dbgNs}provideTm1ExtractionDirectory:authInfoError:`, authInfoError);
            return;
          }) as AuthInfo;
          ...
          if (typeof authInfo === 'undefined') {
            ...
            // Make sure that the AuthInfo's AuthFields has the same Org ID as in the TM1 Analysis JSON.
            SfdxFalconDebug.obj(`${dbgNs}provideTm1ExtractionDirectory:authInfo.AuthFields:`, authInfo.getFields());
            if (authInfo.getFields().orgId !== tm1ExtractionReport.orgInfo['orgId']) {
              foundValidFile = false;
            }
          }
        }
        ...
        // Test whether a Valid File was found. If not, return an error message.
        if (foundValidFile !== true) {
          return 'Directory does not contain a valid tm1-extraction.json file';
        }
        else {
          
          // Path to a valid TM1 Extraction was provided. Save it to Shared Data and allow the user to proceed.
          this.sharedData['tm1ExtractionReport']  = tm1ExtractionReport;
          return true;
        }
      }
    }
