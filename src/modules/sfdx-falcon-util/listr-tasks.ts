...
export function deployTm2Metadata(tm1AnalysisReport:TM1AnalysisReport, tm1ExtractionReport:TM1ExtractionReport, tm1TransformationReport:TM1TransformationReport, tm2DeployFilePaths:TM2DeployFilePaths):ListrObject {
  ...
  // Validate incoming arguments.
  validateTm2DeploymentArguments.apply(null, arguments);
...
export function runPostDeploymentTasks(tmToolsDeploy:TmToolsDeploy):ListrObject {
  ...
  // Validate incoming arguments.
  if (typeof tmToolsDeploy === 'undefined' || tmToolsDeploy === null || (tmToolsDeploy instanceof TmToolsDeploy) !== true)  {
    throw new SfdxFalconError( `Expected tmToolsDeploy to be a non-null instance of TmToolsDeploy.${typeof tmToolsDeploy !== 'undefined' && tmToolsDeploy !== null ? ` Got an instance of '${(tmToolsDeploy.constructor) ? tmToolsDeploy.constructor.name : 'unknown'}' instead.` : '.'}`
                             , `TypeError`
                             , `${dbgNs}fetchTerritory2Ids`);
  }
...
// ────────────────────────────────────────────────────────────────────────────────────────────────┐
/**
 * @function    validateTm2DeploymentArguments
 * @returns     {void}
 * @description Ensures that the arguments provided match an expected, ordered set.
 * @private
 */
// ────────────────────────────────────────────────────────────────────────────────────────────────┘
function validateTm2DeploymentArguments():void {

  // Debug incoming arguments
  SfdxFalconDebug.obj(`${dbgNs}validateTm2DeploymentArguments:arguments:`, arguments);

  // Validate "tm1AnalysisReport".
  if (typeof arguments[0] !== 'object' || arguments[0] === null) {
    throw new SfdxFalconError( `Expected tm1AnalysisReport to be a non-null object but got type '${typeof arguments[0]}' instead.`
                             , `TypeError`
                             , `${dbgNs}validateTm2DeploymentArguments`);
  }
  // Validate "tm1ExtractionReport".
  if (typeof arguments[1] !== 'object' || arguments[1] === null) {
    throw new SfdxFalconError( `Expected tm1ExtractionReport to be a non-null object but got type '${typeof arguments[1]}' instead.`
  ...
  // Validate "tm1TransformationReport".
  if (typeof arguments[2] !== 'object' || arguments[2] === null) {
    throw new SfdxFalconError( `Expected tm1TransformationReport to be a non-null object but got type '${typeof arguments[2]}' instead.`
                             , `TypeError`
                             , `${dbgNs}validateTm2DeploymentArguments`);
  }
  // Validate "tm2DeployFilePaths".
  if (typeof arguments[3] !== 'object' || isEmpty(arguments[3])) {
    throw new SfdxFalconError( `Expected tm2DeployFilePaths to be a non-empty object but got type '${typeof arguments[3]}' instead.`
                             , `TypeError`
                             , `${dbgNs}validateTm2DeploymentArguments`);
  }
}
