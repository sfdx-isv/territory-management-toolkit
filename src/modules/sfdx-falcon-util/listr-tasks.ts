...
import {TM1CleanupFilePaths}      from  '../tm-tools-types';      // Interface. Represents the complete suite of file paths required by the TM1 Cleanup command.
              ...
              task:   (listrContext:object, thisTask:ListrTask) => {
                return new Observable(observer => {
                  ...
                  const otr = initObservableTaskResult(`${dbgNs}analyzeTm1Config:RT1`, listrContext, thisTask, observer, this.sharedData, this.generatorResult,
                              `Saving report to ${tm1Analysis.filePaths.tm1AnalysisReportPath}`);
 ...
 *              TM1 config.
 * @param       {TM1CleanupFilePaths} tm1CleanupFilePaths Required. All file paths needed to
 ...
 */
// ────────────────────────────────────────────────────────────────────────────────────────────────┘
export function cleanTm1Config(tm1AnalysisReport:TM1AnalysisReport, tm1ExtractionReport:TM1ExtractionReport, tm1TransformationReport:TM1TransformationReport, tm1CleanupFilePaths:TM1CleanupFilePaths):ListrObject {
  ...
  typeValidator.throwOnNullInvalidObject(tm1CleanupFilePaths,     `${dbgNsLocal}`, 'tm1CleanupFilePaths');
  ...
  return new listr(
    ...
    [
        ...
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
            ...
            const otr = initObservableTaskResult(`${dbgNsLocal}:CT1`, listrContext, thisTask, observer, this.sharedData, this.generatorResult,
                        `Inspecting transformed TM1 configuration in ${tm1CleanupFilePaths.transformedMetadataDir}`);
      ...
      // ── CLEANUP TASK 2: Destroy all TM1 Sharing Rules ──────────────────────────────────────────
      {
        title:  'Remove orphaned TM1 Sharing Rules',
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
      ...
      // ── CLEANUP TASK 3: Generate a TM1 Cleanup Report ──────────────────────────────────────────
      {
        ...
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
            ...
            // Initialize an OTR (Observable Task Result).
            const otr = initObservableTaskResult(`${dbgNsLocal}:CT3`, listrContext, thisTask, observer, this.sharedData, this.generatorResult,
                        `Writing TM1 Cleanup report to ${tmToolsClean.filePaths.tm1CleanupReportPath}`);
    
            // Define the Task Logic to be executed.
            const asyncTask = async () => {
              const tmToolsCleanupReport = await tmToolsClean.saveReport();
              SfdxFalconDebug.obj(`${dbgNsLocal}:CT3:tmToolsCleanupReport:`, tmToolsCleanupReport);
            };
...
export function deployTm2Metadata(tm1AnalysisReport:TM1AnalysisReport, tm1ExtractionReport:TM1ExtractionReport, tm1TransformationReport:TM1TransformationReport, tm2DeployFilePaths:TM2DeployFilePaths):ListrObject {
  ...
  return new listr(
    ...
    [
      ...
      {
        title:  'Prepare Environment TM2 Metadata Deployment',
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
...
// ────────────────────────────────────────────────────────────────────────────────────────────────┐
/**
 * @function    loadFinalTm2Config
 * @param       {TmToolsLoad} tmToolsLoad Required. Fully-prepared instance of a TmToolsLoad object.
 * @returns     {ListrObject}  A "runnable" Listr Object
 * @description Returns a "runnable" Listr Object that uses a fully-prepared instance of a
 *              TmToolsLoad object to attempt to load/deploy the final set of TM2 config (data and
 *              metadata) into an org with an ACTIVE Territory2 Model.
 * @public
 */
// ────────────────────────────────────────────────────────────────────────────────────────────────┘
export function loadFinalTm2Config(tmToolsLoad:TmToolsLoad):ListrObject {

  // Define function-local debug namespace and validate incoming arguments.
  const dbgNsLocal = `${dbgNs}loadFinalTm2Config`;
  SfdxFalconDebug.obj(`${dbgNsLocal}:arguments:`, arguments);
      ...
      // ── POST-DEPLOYMENT TASK 5: Generate Deployment Report ─────────────────────────────────────
      {
        ...
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
            ...
            // Define the Task Logic to be executed.
            const asyncTask = async () => {
              await tmToolsDeploy.saveReport();
            };
...
export function fetchTm1Data(aliasOrUsername:string, targetDir:string):ListrObject {
  ...
  const groupCsv          = path.join(targetDir,  'Group.csv');
      ...
      // ── SOQL Query 6 ───────────────────────────────────────────────────────────────────────────
      {
        ...
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
            ...
            // Initialize an OTR (Observable Task Result).
            const otr = initObservableTaskResult(`${dbgNs}fetchTm1Data:Q6`, listrContext, thisTask, observer, this.sharedData, this.generatorResult,
            ...
            .then((successResult:SfdxFalconResult) => {
              SfdxFalconDebug.obj(`${dbgNs}fetchTm1Data:Q6:successResult:`, successResult);
            ...
            .catch((failureResult:SfdxFalconResult|Error) => {
              SfdxFalconDebug.obj(`${dbgNs}fetchTm1Data:Q6:failureResult:`, failureResult);
...
export function transformTm1Config(tm1AnalysisReport:TM1AnalysisReport, tm1ExtractionReport:TM1ExtractionReport, tm1TransformFilePaths:TM1TransformFilePaths):ListrObject {
  ...
  // Validate incoming arguments.
  typeValidator.throwOnNullInvalidObject(tm1AnalysisReport,     `${dbgNs}:transformTm1Config`, 'tm1AnalysisReport');
  typeValidator.throwOnNullInvalidObject(tm1ExtractionReport,   `${dbgNs}:transformTm1Config`, 'tm1ExtractionReport');
  typeValidator.throwOnNullInvalidObject(tm1TransformFilePaths, `${dbgNs}:transformTm1Config`, 'tm1TransformFilePaths');
 ...
 * @param       {TM2DataLoadFilePaths} tm2DataLoadFilePaths Required. All file paths needed to
 ...
 */
// ────────────────────────────────────────────────────────────────────────────────────────────────┘
export function validateTm2Activation(tm1AnalysisReport:TM1AnalysisReport, tm1ExtractionReport:TM1ExtractionReport, tm1TransformationReport:TM1TransformationReport, tm2DeploymentReport:TM2DeploymentReport, tm2DataLoadFilePaths:TM2DataLoadFilePaths):ListrObject {
  ...
  typeValidator.throwOnNullInvalidObject(tm2DataLoadFilePaths,    `${dbgNsLocal}`, 'tm2DataLoadFilePaths');
