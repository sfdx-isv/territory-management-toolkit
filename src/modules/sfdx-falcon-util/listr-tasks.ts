...
export function extractTm1Config(tm1AnalysisReport:TM1AnalysisReport, tm1MetadataDir:string, tm1DataDir:string):ListrObject {
  ...
  return new listr(
    ...
    [
      metadataRetrieve.call(this, aliasOrUsername, manifestFilePath, tm1MetadataDir),
 ...
 * @param       {string}  tm1MetadataDir  Required. Path to location of extracted TM1 metadata.
 * @param       {string}  tm1DataDir  Required. Path to location of extracted TM1 data.
 * @param       {string}  tm1ExtractionReportPath Required. Path to file for TM1 Extraction report.
 * @returns     {ListrObject}  A "runnable" Listr Object
 ...
 */
// ────────────────────────────────────────────────────────────────────────────────────────────────┘
export function validateTm1Extraction(tm1AnalysisReport:TM1AnalysisReport, tm1MetadataDir:string, tm1DataDir:string, tm1ExtractionReportPath:string):ListrObject {
      ...
      // ── Validation Task 1 ──────────────────────────────────────────────────────────────────────
      {
        ...
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
            ...
            // Execute the Task Logic
            Tm1Context.validate(tm1AnalysisReport, tm1MetadataDir, tm1DataDir)
        ...
        task:   (listrContext:object, thisTask:ListrTask) => {
          return new Observable(observer => {
            ...
            const otr = initObservableTaskResult(`${dbgNs}validateTm1Extraction:VT7`, listrContext, thisTask, observer, this.sharedData, this.generatorResult,
                        `Writing report to ${tm1ExtractionReportPath}`);
            ...
            const asyncTask = async () => {
              ...
              await fs.writeJson(tm1ExtractionReportPath, tm1ExtractionReport);
            ...
            .then(async (successResult:TM1ExtractionReport) => {
              ...
              thisTask.title += chalk.blue(` (${tm1ExtractionReportPath})`);
              finalizeObservableTaskResult(otr);
            })
            .catch(async (failureResult:Error) => {
...
function validateTm1ExtractionArguments():void {
  ...
  // Validate "tm1Analysis".
  if (typeof arguments[0] !== 'object' || arguments[0] === null) {
    throw new SfdxFalconError( `Expected tm1Analysis to be a non-null object but got type '${typeof arguments[0]}' instead.`
                             , `TypeError`
...
function validateTm1ExtractionValidationArguments():void {
  ...
  // Validate "tm1MetadataDir".
  if (typeof arguments[1] !== 'string' || arguments[1] === '') {
    throw new SfdxFalconError( `Expected tm1MetadataDir to be a non-empty string but got type '${typeof arguments[1]}' instead.`
                             , `TypeError`
                             , `${dbgNs}validateTm1ExtractionValidationArguments`);
  }
  // Validate "tm1DataDir".
  if (typeof arguments[2] !== 'string' || arguments[2] === '') {
    throw new SfdxFalconError( `Expected tm1DataDir to be a non-empty string but got type '${typeof arguments[2]}' instead.`
                             , `TypeError`
                             , `${dbgNs}validateTm1ExtractionValidationArguments`);
  }
  // Validate "tm1ExtractionReportPath".
  if (typeof arguments[3] !== 'string' || arguments[3] === '') {
    throw new SfdxFalconError( `Expected tm1ExtractionReportPath to be a non-empty string but got type '${typeof arguments[3]}' instead.`
                             , `TypeError`
                             , `${dbgNs}validateTm1ExtractionValidationArguments`);
  }
}
