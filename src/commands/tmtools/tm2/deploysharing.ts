//─────────────────────────────────────────────────────────────────────────────────────────────────┐
/**
 * @file          commands/tmtools/tm2/deploysharing.ts
 * @copyright     Vivek M. Chawla - 2019
 * @author        Vivek M. Chawla <@VivekMChawla>
 * @summary       Implements the CLI command `tmtools:tm2:deploysharing`
 * @description   Salesforce CLI Plugin command (`tmtools:tm2:deploysharing`) that allows a Salesforce Admin
 *                to deploy TM2 Sharing Rules metadata from a local directory into an org with TM2
 *                enabled and a migrated `Territory2Model` in the `Active` state. This command MUST
 *                operate on a directory that holds a TM1 Deployment Report (`tm1-deployment.json`).
 * @version       1.0.0
 * @license       MIT
 */
//─────────────────────────────────────────────────────────────────────────────────────────────────┘
...
const dbgNs = 'COMMAND:tmtools-tm2-deploysharing:';
SfdxFalconDebug.msg(`${dbgNs}`, `Debugging initialized for ${dbgNs}`);

// Use SfdxCore's Messages framework to get the message bundles for this command.
Messages.importMessagesDirectory(__dirname);
const commandMessages = Messages.loadMessages('territory-management-toolkit', 'tmtools-tm2-deploysharing');


//─────────────────────────────────────────────────────────────────────────────────────────────────┐
/**
 * @class       TmtoolsTm2DeploySharing
 * @extends     SfdxFalconYeomanCommand
 * @summary     Implements the CLI Command `tmtools:tm2:deploysharing`
 * @description The command `tmtools:tm2:deploysharing`...TODO: Add description
 * @public
 */
//─────────────────────────────────────────────────────────────────────────────────────────────────┘
export default class TmtoolsTm2DeploySharing extends SfdxFalconYeomanCommand {
  ...
  // -s --SOURCEDIR   Directory that contains a tm2-deployment.json file.
  ...
  public async run():Promise<AnyJson> {
    ...
    this.sfdxFalconCommandInit('tmtools:tm2:deploysharing', SfdxFalconCommandType.UNKNOWN);

    // Run a Yeoman Generator to interact with and run tasks for the user.
    await super.runYeomanGenerator({
      generatorType:  'tmtools-tm2-deploysharing',
